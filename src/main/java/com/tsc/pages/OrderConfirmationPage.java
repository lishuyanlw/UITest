...
public class OrderConfirmationPage extends BasePage {
	...
	@FindBy(xpath = "//order-confirmation-cart//div[@class='billing-address-blk']/div[not(@class='billing-address-title')]/div")
	...
	@FindBy(xpath = "//div[@class='order-conf-summary-section']//div[contains(@class,'applied-discount-title')]")
	public WebElement lblOrderSummaryAppliedDiscountTitle;

	@FindBy(xpath = "//div[@class='order-conf-summary-section']//div[contains(@class,'order-conf-summary-row applied-discount')]")
	...
	public boolean checkProductNumberExisting(WebElement productItem){
		...
		return this.checkChildElementExistingByTagNameAndAttribute(item,"div","class","review-item-name");
	}
	...
	public boolean checkAppliedDiscountExistingInOrderSummarySection(){
		return this.checkChildElementExistingByAttribute(cntOrderSummaryContainer,"class","applied-discount-title");
	}
	...
	public Map<String,Object> getOrderSummaryDesc(){
		...
		if(checkAppliedDiscountExistingInOrderSummarySection()){
			if(lstOrderSummaryAppliedDiscountList.size()==2){
				item=this.lstOrderSummaryAppliedDiscountList.get(0);
				this.getReusableActionsInstance().javascriptScrollByVisibleElement(item);
				subItem=item.findElement(By.xpath(".//div[@class='order-conf-summary-lbl']"));
				lsText=subItem.getText().trim();
				map.put("promoteCodeTitle",lsText);
				subItem=item.findElement(By.xpath(".//div[@class='order-conf-summary-val']"));
				lsText=subItem.getText().trim();
				map.put("promoteCodeValue",this.getFloatFromString(lsText,true));

				item=this.lstOrderSummaryAppliedDiscountList.get(1);
				this.getReusableActionsInstance().javascriptScrollByVisibleElement(item);
				subItem=item.findElement(By.xpath(".//div[@class='order-conf-summary-lbl']"));
				lsText=subItem.getText().trim();
			...
			if(lstOrderSummaryAppliedDiscountList.size()==1){
				item=this.lstOrderSummaryAppliedDiscountList.get(0);
				this.getReusableActionsInstance().javascriptScrollByVisibleElement(item);
				subItem=item.findElement(By.xpath(".//div[@class='order-conf-summary-lbl']"));
				lsText=subItem.getText().trim();
		...
		if(this.checkAppliedDiscountExistingInOrderSummarySection()){
			...
			for(WebElement item:lstOrderSummaryAppliedDiscountList){
				...
				if(!lsText.isEmpty()){
					...
					reporter.reportLogFail("Applied discount item title is not displaying correctly");
				}
				...
				if(!lsText.isEmpty()){
					...
					reporter.reportLogFail("Applied discount item value is not displaying correctly");
				}
			}
		}
