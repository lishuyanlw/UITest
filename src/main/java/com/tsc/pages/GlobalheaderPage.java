...
public class GlobalheaderPage extends BasePage{
	...
	*/						
	...
	public boolean validateSubMenuHeadings(String section) { 
		//getReusableActionsInstance().scrollToElement(webElement);
	...
	public String verify_Header_SubMenu(String sectionName, String attribute){
		String textMissing= null;
		switch(sectionName){
		  	  case "SubMenu":
		  		textMissing=verify_subMenu_Item(this.listSubItems,sectionName,attribute);
		    	  return textMissing;
		        // break;
		      case "CuratedCollection":
		    	  textMissing =verify_subMenu_Item(this.listSubItemsCuratedCollection,sectionName,attribute);
		    	  return textMissing;
		       //  break;
		      case "PopularBrands":
		    	  textMissing =verify_subMenu_Item(this.listSubItemsPopularBrands,sectionName,attribute);
		    	  return textMissing;
		        // break;
		   }
		   return textMissing;
	...
	/*Method to verify href/src is empty or not before clicking link
	  ...
	  * @author Shruti Desai
	  */
	public String verify_subMenu_Item(List<WebElement> webElements, String sectionName, String attribute) {
		StringBuilder href_src  =new StringBuilder();
		System.out.println("size of List<WebElement>:------>"+webElements.size()+" for "+sectionName);
				...
				case "PopularBrands":
					...
					if(attribute=="href") {//href is not present
						...
						if(verifyElementProperty(hreffattri,"Link")) {//href not present
							href_src.append(element.getAttribute("alt")).append('\n');
						}
					...
					break;
					case "CuratedCollection":	
						System.out.println("I am in CuratedCollection section");
						if(verifyElementProperty(element,"Link")) {//href is not present
							href_src.append(element.getText()).append('\n');
						} 
					break;
					case "SubMenu":
						System.out.println("I am in SUBMENU section");
						//WebElement itemSectionElement =element.findElement(By.xpath("./ancestor::ul"));
						//if(getChildElementCount(itemSectionElement)>0) {
							if(verifyElementProperty(element,"Link")) {//href is not present
							href_src.append(element.getText()).append('\n');
							} 
						//}	
