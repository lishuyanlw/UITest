...
public class GF_TC02_Verify_GlobalFooter_CustomerHubLinksAndAboutTSCLinks_Language extends BaseTest {
	...
	public void Verify_GlobalFooter_CustomerHubLinksAndAboutTSCLinks_Language() throws IOException {
		...
		//Gift card
		lsText=basePage.getElementText(getGlobalFooterPageThreadLocal().lnkGiftCard);
		String lsText_GiftCard_En=getGlobalFooterPageThreadLocal().getEnglishWithSpecificFrenchName(lstNameAndLinks,lsText_GiftCard_Fr);
		reporter.softAssert(lsText.equalsIgnoreCase(lsText_GiftCard_En),"The English text of "+lsText+" is correct","The English text of "+lsText+" is not correct");
		lsYmlHref=getGlobalFooterPageThreadLocal().getLinkWithSpecificName(lstNameAndLinks,lsText,true);
		if(lsYmlHref.isEmpty()) {
			reporter.reportLog("Unable to find Gift card link in yml file.");
		}
		...
		//Send us feedback
		lsText=basePage.getElementText(getGlobalFooterPageThreadLocal().lnkSendUsFeedback);		
		String lsText_SendUsFeedback_En=getGlobalFooterPageThreadLocal().getEnglishWithSpecificFrenchName(lstNameAndLinks,lsText_SendUsFeedback_Fr);
		reporter.softAssert(lsText.equalsIgnoreCase(lsText_SendUsFeedback_En),"The English text of "+lsText+" is correct","The English text of "+lsText+" is not correct");		
		lsYmlHref=getGlobalFooterPageThreadLocal().getLinkWithSpecificName(lstNameAndLinks,lsText,true);
		if(lsYmlHref.isEmpty()) {
			reporter.reportLog("Unable to find Send us feedback link in yml file.");
		}
		...
		//Language switch
		lsText=basePage.getUTFEnabledData(basePage.getElementText(getGlobalFooterPageThreadLocal().lnkLanguage));		
		String lsText_LanguageSwitch_Fr=getGlobalFooterPageThreadLocal().getEnglishWithSpecificFrenchName(lstNameAndLinks,lsText_LanguageSwitch_En);
		reporter.softAssert(lsText.equalsIgnoreCase(lsText_LanguageSwitch_Fr),"The English text of "+lsText+" is correct","The English text of "+lsText+" is not correct");			
		lsYmlHref=getGlobalFooterPageThreadLocal().getLinkWithSpecificName(lstNameAndLinks,lsText,true);
		if(lsYmlHref.isEmpty()) {
			reporter.reportLog("Unable to find Language switch link in yml file.");
		}
		...
		//TSC customer hub links
		lsText=basePage.getUTFEnabledData(basePage.getElementText(getGlobalFooterPageThreadLocal().lblTSCCustomerHubText));		
		String lsText_TSCCustomerHub_En=getGlobalFooterPageThreadLocal().getEnglishWithSpecificFrenchName(lstNameAndLinks,lsText_TSCCustomerHub_Fr);
		reporter.softAssert(lsText.equalsIgnoreCase(lsText_TSCCustomerHub_En),"The English text of "+lsText+" is correct","The English text of "+lsText+" is not correct");			
			
		WebElement item;
		String lsEn;
		for(int i=0;i<getGlobalFooterPageThreadLocal().lnkTSCCustomerHubAllLinks.size();i++) {
			item=getGlobalFooterPageThreadLocal().lnkTSCCustomerHubAllLinks.get(i);
			lsText=basePage.getUTFEnabledData(basePage.getElementText(item));	
			lsFr=lstCustomerHubFr.get(i);			
			lsEn=getGlobalFooterPageThreadLocal().getEnglishWithSpecificFrenchName(lstNameAndLinks,lsFr);
			reporter.softAssert(lsText.equalsIgnoreCase(lsEn),"The English text of "+lsText+" is correct","The English text of "+lsText+" is different from "+lsEn);
			lsYmlHref=getGlobalFooterPageThreadLocal().getLinkWithSpecificName(lstNameAndLinks,lsText,true);			
			if(lsYmlHref.isEmpty()) {
				reporter.reportLog("Unable to find "+lsText+" link in yml file.");
			}
		...
		//About TSC links
		lsText=basePage.getUTFEnabledData(basePage.getElementText(getGlobalFooterPageThreadLocal().lblAboutTSCText));		
		String lsText_AboutTSC_En=getGlobalFooterPageThreadLocal().getEnglishWithSpecificFrenchName(lstNameAndLinks,lsText_AboutTSC_Fr);
		reporter.softAssert(lsText.equalsIgnoreCase(lsText_AboutTSC_En),"The English text of "+lsText+" is correct","The English text of "+lsText+" is not correct");			
				
		for(int i=0;i<getGlobalFooterPageThreadLocal().lnkAboutTSCAllLinks.size();i++) {
			item=getGlobalFooterPageThreadLocal().lnkAboutTSCAllLinks.get(i);
			lsText=basePage.getUTFEnabledData(basePage.getElementText(item));
			lsFr=lstAboutTSCFr.get(i);
			lsEn=getGlobalFooterPageThreadLocal().getEnglishWithSpecificFrenchName(lstNameAndLinks,lsFr);
			reporter.softAssert(lsText.equalsIgnoreCase(lsEn),"The English text of "+lsText+" is correct","The English text of "+lsText+" is different from "+lsEn);
			lsYmlHref=getGlobalFooterPageThreadLocal().getLinkWithSpecificName(lstNameAndLinks,lsText,true);			
			if(lsYmlHref.isEmpty()) {
		...
		//Copyright text
		lsText=basePage.getUTFEnabledData(basePage.getElementText(getGlobalFooterPageThreadLocal().txtCopyrightLine1));		
		String lsText_Copyright_Line1_En=getGlobalFooterPageThreadLocal().getEnglishWithSpecificFrenchName(lstNameAndLinks,lsText_Copyright_Line1_Fr);
		reporter.softAssert(lsText.equalsIgnoreCase(lsText_Copyright_Line1_En),"The English text of "+lsText+" is correct","The English text of "+lsText+" is not correct");		
		lsText=basePage.getUTFEnabledData(basePage.getElementText(getGlobalFooterPageThreadLocal().txtCopyrightLine2));	
		String lsText_Copyright_Line2_En=getGlobalFooterPageThreadLocal().getEnglishWithSpecificFrenchName(lstNameAndLinks,lsText_Copyright_Line2_Fr);
		reporter.softAssert(lsText.equalsIgnoreCase(lsText_Copyright_Line2_En),"The English text of "+lsText+" is correct","The English text of "+lsText+" is not correct");			
