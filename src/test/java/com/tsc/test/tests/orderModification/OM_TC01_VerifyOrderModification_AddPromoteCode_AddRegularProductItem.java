...
public class OM_TC01_VerifyOrderModification_AddPromoteCode_AddRegularProductItem extends BaseTest {
    /*
     * CER-904
     */
    @Test(groups={"Regression","OrderModification"})
    public void OM_TC01_VerifyOrderModification_AddPromoteCode_AddRegularProductItem() throws IOException {
        ...
        if(getGivenOrderResponse==null){
            placeOrderResponse = getMyAccountPageThreadLocal().placeOrderForUser(Integer.parseInt(customerEDP),accessToken,itemsToBeAdded,2,"1",true,517281);
        ...
        if(System.getProperty("Device").equalsIgnoreCase("Desktop"))
            ...
            final String pdpPageURL = apiClient.getApiPropertyData().get("test_qaURL") + "/" + product.getName().trim() + apiClient.getApiPropertyData().get("test_partial_url_pdp") + product.getItemNo();
            try{
